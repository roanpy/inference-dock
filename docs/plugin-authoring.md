...
a shell command. `service.catalog_args` is an optional argv suffix for a
trusted executable's read-only local model catalog; `service.models_path`
provides the equivalent probe for an already-running HTTP service when that
route returns its complete local catalog rather than only loaded models. Both are
discovery evidence only and never download, delete, start, or reconfigure a
model. Runtime model entries may use an explicit `catalog_id` and
`asset_paths`; asset checks stat only those paths and never scan a directory.

Adapters that can release models used by direct clients should also declare
safe `service.active_requests_path` and `service.model_state_path` probes.
They must report actual in-flight requests and resident models; `/v1/models`
is not a valid state probe when it only lists the model catalog.
