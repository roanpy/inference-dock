...
- Quitting the menu bar leaves a dispatcher core started by the app running;
  the explicit stop-core action terminates only that owned process and does not
  affect external services or in-flight requests owned by another process.
- The native Settings window exposes config/reload/reconcile checks, instance
  and capability previews, policy controls, an Agent endpoint preview, and
  update diagnostics. Server import preview accepts only an explicitly chosen
  loopback endpoint, executable, source directory, or supported config file;
  it never scans or writes `engines.yaml`. Import/delete/provider-apply/rollback
  actions remain disabled and are labeled as API gaps.
