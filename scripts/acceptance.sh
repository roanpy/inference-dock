...
swift build
for pattern in 'modelManagement' 'systemSettings' 'checkUpdates' 'runHistory' 'runtimeDetails' 'stopServiceUnavailable' 'state.external' 'state.disabled' 'details.configured' 'details.effective' 'maxOutputTokens' 'max' 'median' 'unknown' 'MetricsPayload' '/v1/metrics' 'update_monitor.py' 'SMAppService' 'loginLaunchPreferred' '/v1/settings' '/v1/cancel' 'adapter_policies' 'model_policies' 'exclusive_groups' 'estimated_memory_gb'; do
