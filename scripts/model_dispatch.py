...
class ModelDispatcher:
    ...
    def reload_config(self) -> dict[str, Any]:
        ...
        """
        with self.condition:
            if self._busy() or self.active_requests:
        ...
        for adapter in self.config.adapters.values():
            ...
            try:
                if not backend.healthy():
                    results.append({"adapter": adapter.name, "plugin_id": adapter.plugin_id, "status": "unreachable", "configured": configured_entries})
                    continue
                ...
                if status != 200 or not isinstance(payload, dict) or not isinstance(payload.get("data"), list):
                    results.append({"adapter": adapter.name, "plugin_id": adapter.plugin_id, "status": "invalid_response", "configured": configured_entries})
                    continue
                ...
                results.append({
                    ...
                    "new_candidates": [
                        {"id": candidate, "source": "service-discovered"}
                        for candidate in new_candidates
                    ],
                    "missing_candidates": [
                        {"id": entry["id"], "model": entry["model"], "enabled": entry["enabled"], "advertise": entry["advertise"]}
                        for entry in configured_entries
                        if entry["id"] in missing_candidates
                    ],
                })
            except DispatchError as exc:
                results.append({"adapter": adapter.name, "plugin_id": adapter.plugin_id, "status": "error", "error": exc.message, "configured": configured_entries})
        ...
        try:
            ...
            if cleanup:
                ...
                try:
                    ...
                    self._unload_service(adapter, keep_service_running=bool(model.deactivate_path), excluding=model.name)
