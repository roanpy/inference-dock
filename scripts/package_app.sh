...
FINAL_APP_INPUT="${MODEL_DISPATCH_APP_OUTPUT:-$ROOT/dist/InferenceDock.app}"
APP_NAME=$(basename "$FINAL_APP_INPUT")
APP_PARENT_INPUT=$(dirname "$FINAL_APP_INPUT")
mkdir -p "$APP_PARENT_INPUT"
APP_PARENT=$(CDPATH= cd -- "$APP_PARENT_INPUT" && pwd -P)
FINAL_APP="$APP_PARENT/$APP_NAME"
BUILD_ROOT=$(mktemp -d "$APP_PARENT/.${APP_NAME}.tmp.XXXXXX")
APP="$BUILD_ROOT/$APP_NAME"
PREVIOUS_APP="$FINAL_APP.previous"
cleanup() {
  rm -rf "$BUILD_ROOT"
}
...
cp scripts/agent_config.py "$APP/Contents/scripts/agent_config.py"
...
# Bind Info.plist and resources for local builds. Public distribution still
# requires a Developer ID signature and Apple notarization.
codesign --force --deep --sign - "$APP" >/dev/null

# Test-only fault injection used by acceptance to prove an existing bundle is
# untouched when staging fails after assembly. It is never set by normal builds.
if [ "${MODEL_DISPATCH_TEST_FAIL_BEFORE_INSTALL:-0}" = "1" ]; then
  echo "test fault injection: refusing staged app install" >&2
  exit 77
fi

# Replace only after the temporary bundle has passed build, signing and public
# safety checks. Keep the previous bundle as an exact sibling rollback point.
...
if ! mv "$APP" "$FINAL_APP"; then
  ...
  echo "Failed to install assembled app; previous app was restored when possible" >&2
  exit 1
fi
