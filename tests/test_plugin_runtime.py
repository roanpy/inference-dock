...
def test_runtime_compiles_lifecycle_probes():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        path.write_text("\n".join([
            ...
            "service: {mode: external, endpoint: http://127.0.0.1:19000, active_requests_path: /activity, model_state_path: /loaded}",
            "models_source: none",
        ]), encoding="utf-8")
        manifest = plugin_registry.load_manifest(path)
        adapter = plugin_runtime.compile_runtime([manifest], [{"id": "probe"}])["adapters"]["probe"]
        assert adapter["active_requests_path"] == "/activity"
        assert adapter["model_state_path"] == "/loaded"
        assert adapter["platform"]["id"] == "probe"
        assert adapter["instance"]["mode"] == "external"


def test_runtime_deduplicates_models_by_instance_and_backend_id():
    manifest = plugin_registry.load_manifest(ROOT / "plugins/ds4/manifest.yaml", trusted=True)
    fields = {
        "variants": {"qwen": {"model": "qwen", "context": 4096}},
        "services": {
            "a": {"model": "qwen"},
            "b": {"model": "qwen"},
        },
    ...
    compiled = plugin_runtime.compile_runtime(
        [manifest],
        [{"id": "ds4", "instance": {"id": "ds4:shared"}, "import_preview": [{"fields": fields}]}],
    )
    assert set(compiled["models"]) == {"qwen-a"}
    assert compiled["models"]["qwen-a"]["backend_model"] == "qwen"
    assert compiled["models"]["qwen-a"]["aliases"] == ["qwen-b", "qwen"]
...
def test_ds4_takeover_compiles_managed_adapters():
    ...
    assert model["asset_paths"] == [
        f"{root}/fixture-high.gguf",
        f"{root}/fixture-vision.gguf",
        f"{root}/fixture-aux.gguf",
    ]
...
def test_model_entries_expose_instance_and_canonical_fields():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            assert entries["m1"]["server_type"] == "external"
...
def test_lifecycle_template_marks_honest_gaps():
    mlx = plugin_registry.load_manifest(ROOT / "plugins/mlx-serve/manifest.yaml", trusted=True)
    lifecycle = plugin_runtime.lifecycle_template(mlx)
    assert set(lifecycle) == set(plugin_runtime.LIFECYCLE_ACTIONS)
    assert lifecycle["discover"]["available"] is True
    assert lifecycle["health"]["available"] is True
    assert lifecycle["load"]["available"] is True and lifecycle["load"]["detail"]["path"] == "/v1/load-model"
    assert lifecycle["unload"]["available"] is True
    # external services are never started/stopped by InferenceDock
    assert lifecycle["start"]["available"] is False and "启动" in lifecycle["start"]["note_zh"]
    assert lifecycle["stop"]["available"] is False and "不停止" in lifecycle["stop"]["note_zh"]
    # missing probes are explicit fail-closed gaps
    assert lifecycle["active_requests"]["available"] is False and "忙碌" in lifecycle["active_requests"]["note_zh"]
    assert lifecycle["model_state"]["available"] is False and "失败" in lifecycle["model_state"]["note_zh"]
    assert lifecycle["cancel"]["available"] is False

    omlx = plugin_registry.load_manifest(ROOT / "plugins/omlx/manifest.yaml", trusted=True)
    observe = plugin_runtime.lifecycle_template(omlx)
    assert observe["load"]["available"] is False and observe["unload"]["available"] is False
    assert observe["start"]["available"] is False and observe["stop"]["available"] is False

    # Ollama's verified /api/ps probe makes model_state available; the missing
    # activity probe stays an explicit fail-closed gap.
    ollama = plugin_registry.load_manifest(ROOT / "plugins/ollama/manifest.yaml", trusted=True)
    ollama_lifecycle = plugin_runtime.lifecycle_template(ollama)
    assert ollama_lifecycle["model_state"]["available"] is True
    assert ollama_lifecycle["model_state"]["detail"]["path"] == "/api/ps"
    assert ollama_lifecycle["active_requests"]["available"] is False
    assert "忙碌" in ollama_lifecycle["active_requests"]["note_zh"]

    # oMLX keep_resident propagates into the compiled adapter preview.
    compiled = plugin_runtime.compile_runtime([omlx], [{"id": "omlx"}])
    assert compiled["adapters"]["omlx"]["keep_resident"] is True
    assert compiled["adapters"]["omlx"]["type"] == "observe"

    llama = plugin_registry.load_manifest(ROOT / "plugins/llama-cpp/manifest.yaml", trusted=True)
    draft = plugin_runtime.lifecycle_template(llama)
    assert draft["start"]["available"] is False and "草稿" in draft["start"]["note_zh"]
    assert draft["load"]["available"] is False and "不做能力假设" in draft["load"]["note_zh"]

    # The mlx-lm / mlx-vlm / vllm-mlx templates are pure discovery drafts:
    # every service action is an honest gap, never a guessed route.
    for plugin_id in ("mlx-lm", "mlx-vlm", "vllm-mlx"):
        manifest = plugin_registry.load_manifest(ROOT / f"plugins/{plugin_id}/manifest.yaml", trusted=True)
        lifecycle = plugin_runtime.lifecycle_template(manifest)
        assert lifecycle["discover"]["available"] is True
        for action in ("health", "list_models", "start", "load", "unload", "stop"):
            assert lifecycle[action]["available"] is False, (plugin_id, action)
        compiled = plugin_runtime.compile_runtime([manifest], [{"id": plugin_id}])
        assert compiled["adapters"][plugin_id]["unresolved"] == ["start_argv"]

    # A managed manifest with verified argv/stop gets start/stop actions.
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "manifest.yaml"
        path.write_text("\n".join([
            "schema_version: 1",
            "id: full",
            "display_name: Full",
            "discover: {executables: [\"full-bin\"]}",
            "service: {mode: managed, endpoint: http://127.0.0.1:19001, health_path: /health}",
            "ownership: {mode: managed, start_argv: [\"/abs/full-bin\", \"serve\"], stop_strategy: process-group}",
            "models_source: none",
        ]), encoding="utf-8")
        manifest = plugin_registry.load_manifest(path)
        lifecycle = plugin_runtime.lifecycle_template(manifest)
        assert lifecycle["start"]["available"] is True and lifecycle["start"]["detail"] == ["/abs/full-bin", "serve"]
        assert lifecycle["stop"]["available"] is True


def test_plan_includes_lifecycle_for_every_builtin():
    manifests, errors = plugin_registry.load_manifests()
    assert not errors
    for manifest in manifests:
        plan = plugin_runtime.plan_actions(manifest, {"id": manifest.id, "import_preview": []})
        lifecycle = plan["lifecycle"]
        assert set(lifecycle) == set(plugin_runtime.LIFECYCLE_ACTIONS)
        assert all(set(action) == {"available", "kind", "detail", "note_zh"} for action in lifecycle.values())
        assert all(isinstance(action["note_zh"], str) and action["note_zh"] for action in lifecycle.values())
        assert lifecycle["cancel"]["available"] is False
...
TESTS = [
    ...
    test_runtime_compiles_lifecycle_probes,
    ...
    test_lifecycle_template_marks_honest_gaps,
    test_plan_includes_lifecycle_for_every_builtin,
]
