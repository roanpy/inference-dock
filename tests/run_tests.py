...
def test_reconcile_models():
    with tempfile.TemporaryDirectory(prefix="model-dispatch-reconcile-test-") as tmp:
        ...
        try:
            ...
            assert managed["new_candidates"] == ["fake-model"]
            assert managed["missing_candidates"] == ["model-a"]
